...
function loadGames(data) {
	...
	for (let i = 0; i < data.length; i++) {
		...
		if (starredgames.includes(data[i].directory)) {
			$element.find("img-big.star").attr("id", "starred");
			$element.find("img-big.star").attr("src", "img-big/star-fill.svg");
	...
	$(document).on("click", ".game", function (event) {
		if ($(event.target).is("img-big.star")) {
			if (!$(event.target).attr("id")) {
				$(event.target).prop({ id: "starred" });
				$(event.target).prop({ src: "img-big/star-fill.svg" });
				...
				$(event.target).attr("src", "img-big/star.svg");
				...
				$($thisdiv + " #starred").attr("src", "img-big/star.svg");
